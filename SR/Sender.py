        ...
        while True:
            try:
                ...
                if ackno == base:
                    base += 1
                    ackno += 1
                    while ackno in ack_window:
                        base += 1
                        ackno += 1
                else:
                    ack_window.add(ackno)
                break
            except socket.timeout:
                timeout_time = time.time()
                for iseq, itime in timers.items():
                    if timeout_time-itime >= retry_timeout/1000:
                        eof = 1 if iseq == len(file_data)-1 else 0
                        packet = make_packet(iseq, eof, file_data[iseq])
                        timers[iseq] = time.time()
